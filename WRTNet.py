...
class LightBlock(nn.Module):
    """
    Lightweight residual block for local feature enhancement.
    """
...
class WaveletTransform2D(nn.Module):
    """
    Differentiable 2D wavelet decomposition and reconstruction using ptwt.
    """
...
class WaveProcessBlock(nn.Module):
    """
    Multiscale wavelet feature-processing block.

    The low-frequency component is enhanced independently. Directional
    features derived from it guide refinement of the three high-frequency
    components before inverse wavelet reconstruction.
    """
    ...
    def forward(self, x):
        ...
        # Restore the input size when odd dimensions introduce a one-pixel offset.
        x = x[:, :, :H0, :W0]
...
class CWRefineNet(nn.Module):
    """
    Wavelet refinement network operating on high-resolution decoder features.

    Both input and output have shape [B, in_ch, H, W]. The module refines
    latent features rather than already reconstructed Vp, Vs, and density.
    """
...
class Transfomerdecoder(nn.Module):
    def __init__(
        ...
        # Enable wavelet refinement in the shared high-resolution feature space.
        ...
        # =====================================================
        # Input and patch-grid dimensions.
        # =====================================================
        self.batch_size = batch_size
        self.nt = nt
        self.nr = nr
        ...
        # =====================================================
        # Fuse the two seismic components.
        ...
        # =====================================================
        # Project Transformer tokens into the latent vector shared by all decoders.
        ...
        # =====================================================
        # Three parallel decoders generate high-resolution latent features.
        # They do not directly output Vp, Vs, or density.
        # =====================================================
        ...
        if (
            ...
            * initial_shape_decoder[1]
            ...
            raise ValueError(
                "final_size_encoder must equal "
                "initial_shape_decoder[0] * "
                "initial_shape_decoder[1], but got "
        ...
        # =====================================================
        # Concatenate the three decoder branches before wavelet refinement.
        # [B,F,H,W] * 3 -> [B,3F,H,W]
        # =====================================================
        ...
        # The only layer that maps latent features to Vp, Vs, and density.
    ...
    def forward(self, xx, yy):
        ...
        # Extract global features with the Transformer encoder.
        x = self.blocks(x)
        x = self.norm(x)
        # x: [B,N,C]
        ...
        if N != self.num_patches:
            raise RuntimeError(
                f"Unexpected Transformer token count: "
                f"N={N}, num_patches={self.num_patches}. "
                "Check nt, nr, and patch_size."
            )

        # Convert Transformer tokens into the shared latent vector.
        x = x.reshape(B, N * C)
        x = self.final(x)
        # x: [B,final_size_encoder]

        # Generate high-resolution latent features in three parallel branches.
        f_vp = self.decoder_vp_feature(x)
        f_vs = self.decoder_vs_feature(x)
        f_rho = self.decoder_rho_feature(x)
        # Each branch: [B,decoder_feature_ch,H_model,W_model]
        ...
        # Concatenate the three high-resolution feature branches.
        feature_3branch = torch.cat([f_vp, f_vs, f_rho], dim=1)
        # [B,3*decoder_feature_ch,H_model,W_model]

        # Refine the combined high-resolution features in the wavelet domain.
        feature_refined = self.wave_refine(feature_3branch)
        # The refinement preserves the feature shape.

        # Produce the three parameter perturbations once, after refinement.
